<#
.SYNOPSIS
    Runs the yuuka_wakeword_v1 pipeline unattended: setup -> generate -> augment ->
    train -> export -> eval.

.DESCRIPTION
    Meant to be started before bed and left running. Per TRAINING.md's measured
    time-budget table, `generate` alone is ~4.5h for 10,000 positive clips on an
    RTX 3070, and negatives/val/backgrounds add more on top -- this config
    (configs/yuuka_v1.yaml) realistically spans one long night, maybe two.
    `generate` is resumable either way (it counts existing clip_NNNNNN.wav files
    and picks up where it left off), so if a second night is needed, just
    re-running this script continues rather than restarting. `train` checkpoints
    periodically within each phase too.

    Stops immediately if any stage exits non-zero, rather than plowing into the
    next stage on broken output.
...
.EXAMPLE
    ...
    To run detached, so closing this terminal doesn't kill it:
        Start-Process powershell -WindowStyle Hidden -ArgumentList `
            "-NoProfile","-ExecutionPolicy","Bypass","-File","wakeword_training\run_v1_overnight.ps1"
...
$config = "wakeword_training/configs/yuuka_v1.yaml"
$logDir = "wakeword_training/logs"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$log = Join-Path $logDir "yuuka_wakeword_v1_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
...
Add-Content -Path $log -Value "===== $(Get-Date -Format 'u')  starting yuuka_wakeword_v1 run ====="
...
Write-Host $doneLine
Write-Host "Model: wakeword_training/output/yuuka_wakeword_v1/yuuka_wakeword_v1.onnx"
Write-Host "Eval:  wakeword_training/output/yuuka_wakeword_v1/yuuka_wakeword_v1_eval.json"
